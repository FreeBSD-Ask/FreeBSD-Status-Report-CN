...
在第二季度，我们新增了 7 名提交者，并收回了 1 个提交位。
...
对于 FreeBSD 手册，完成了“前言”和“打印”部分的翻译工作。也更新了“linuxemu”和“serialcomms”部分。目前，“bsdinstall”、“cutting-edge”、“desktop”、“install”、“introduction”、“kernelconfig”、“mirrors”、“multimedia”、“pgpkeys”、“ports”、“printing”和“x11”章节与英文版本同步了。
...
2. 人们希望在 redports 上有 GCC 测试环境，在该环境中所有 Port 都使用 **lang/gcc47** 构建。为了实现这一点，我们需要修补 Port 框架以处理这一需求，并正确地使用基础 GCC 进行引导。这还使我们能够使用现代 GCC 构建所有二进制包，并且对普通用户而言易于使用。有贡献者吗？
...
2. 将 xorg 分发版更新为开发库中的版本。xorg 项目最近发布了新版本，而开发库中包含该版本。此版本需要更多的测试，才能合并，并且 6 月初发出了 CFT。该项工作仍在进行中。
3. 决定如何处理新旧 xorg 分发版。在新版 xorg 中，很多遗留的驱动支持已丢弃，因此我们需要维护两个 xorg 分发版，以避免丧失大量硬件驱动。目前，通过在 **/etc/make.conf** 中设置 `WITH_NEW_XORG` 标志来完成此操作，但需要更实用的解决方案。特别是考虑到该标志不够用户友好，并且目前没有官方包支持新版分发版。
