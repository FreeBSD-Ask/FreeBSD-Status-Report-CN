...
此外，我们还提供了一些关于 [13.2-RELEASE](https://www.freebsd.org/releases/13.2R/announce/) 的信息。该版本原定于 2023 年第一季度末发布，但由于本报告发布时新版本已经释出，因此用户现在可以直接安装。RELEASE 版本的用户现在可以享受到诸多改进，例如对 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动的更好支持，以及全新的 [rtw88(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw88&sektion=4&format=html) 驱动等，这些主题在过去的状态报告中已有涉及。  
...
来自各个官方和半官方团队的报告，可在 [管理页面](https://www.freebsd.org/administration/) 找到。  

### FreeBSD Core 团队  

**联系人**：FreeBSD Core 团队 \<[core@FreeBSD.org](mailto:core@FreeBSD.org)\>  

FreeBSD Core 团队是 FreeBSD 的管理机构。  

#### 事项  

##### Core 团队章程：草案  

在 2023 年的第一次 Core 团队会议上，来自 2022 年 12 月美国博尔德会议的团队代表向整个团队汇报了代表团的结论。团队将继续讨论相关问题，并与 FreeBSD 基金会合作推进。  

##### FreeBSD 年度开发者调查  

Core 团队与 FreeBSD 基金会达成一致，由 FreeBSD 基金会负责开展年度开发者调查。  

##### Matrix 即时通讯方案  

Core 团队正在持续评估 Matrix 作为 FreeBSD 开发者的即时通讯（IM）解决方案。目前已部署了一个实例，并正在进行测试。  

#### Commit 权限  

* Core 批准了 Cheng Cui (cc@) 的 src commit 权限。  
* Core 批准恢复 Joseph Koshy (jkoshy@) 的 src commit 权限。  
...
如果您希望帮助我们继续推进 FreeBSD 相关工作，请考虑为我们的 **2023 年筹款活动** 捐款：[https://www.freebsdfoundation.org/donate/](https://www.freebsdfoundation.org/donate/)  
...
在 2023 年第一季度，共有 **226** 个 src、**39** 个 ports 和 **12** 个 doc 树的提交标注了基金会作为赞助方。其中部分工作在单独的报告条目中进行了详细介绍：  
...
基金会为 FreeBSD 项目提供了一名全职员工，并资助多个项目，以改进持续集成（CI）、自动化测试以及整体质量保证工作。您可以在专门的报告条目中了解更多 CI 相关工作。  
...
我们通过发布专业制作的 **FreeBSD 期刊** 来教育全球用户了解 FreeBSD。如前所述，FreeBSD 期刊现已免费发行。您可以在 [https://www.freebsdfoundation.org/journal/](https://www.freebsdfoundation.org/journal/) 了解更多信息并获取最新期刊。
...
基金会拥有 FreeBSD 的商标，保护这些商标是我们的责任。同时，我们也为 Core 团队提供法律支持，以调查相关问题。

请访问 [https://www.freebsdfoundation.org](https://www.freebsdfoundation.org/) 了解更多关于我们如何支持 FreeBSD以及我们能为您提供哪些帮助的信息！
...
目前我们已经注意到，在全球互联网交换中心（如澳大利亚、巴西和南非）设立单机镜像是一种不错的模式；如果您认识或在这些中心工作的人员能够赞助一台单机镜像服务器，请与我们联系。美国（西海岸）和欧洲（任一地点）是优选区域。
...
有关更多正在进行中的信息，请参阅 [freebsd-testing@ 相关的 Bugzilla 工单](https://bugs.freebsd.org/bugzilla/buglist.cgi?bug_status=%3Cem%3Eopen%3C/em%3E&email1=testing%40FreeBSD.org&emailassigned_to1=1&emailcc1=1&emailtype1=equals)，并欢迎您加入我们的工作！
...
- 目前 Ports Tree 中大约有 33,500 个端口。针对这些端口，共有 3,021 个未解决的问题报告，其中有 764 个尚未分配。
- 今年前三个月，`main` 分支有 163 位提交者提交了 9,021 次提交，而 `2023Q1` 分支有 55 位提交者提交了 701 次提交。与 `2022Q4` 相比，这意味着端口数量、端口 PR、端口提交次数以及活跃的端口提交者都有小幅增长。
...
- 改善内核模块二进制软件包的现状；
- 如何衡量端口对其依赖项的影响以及如何维护高影响力的端口。

本季度还进行了 32 次 exp-run，用于测试端口更新、更改默认版本（例如：LLVM 升级至 15、MySQL 升级至 8.0、Ruby 升级至 3.1）以及在 base 中更新 byacc。此外，Go 的默认版本切换到了 1.20，而 Lazarus 的默认版本切换到了 2.2.6。

引入了四个新的 USES 变量：

- `budgie`：支持与 Budgie 桌面相关的端口；
- `ldap`：为 OpenLDAP 提供支持，默认版本更新为 26（即 2.6）；
- `nextcloud`：支持 Nextcloud 应用程序；
- `ruby`：为 Ruby 端口提供支持（原先为 `bsd.ruby.mk`）。
...
- \<[freebsd-status-calls@FreeBSD.org](mailto:freebsd-status-calls@FreeBSD.org)\>：这是一个邮件列表，您可以 [订阅](https://lists.freebsd.org/subscription/freebsd-status-calls) 以获取状态报告提交截止日期的提醒。
...
目前正在进行一项改进 `daemon` 工具代码质量和监控能力的工作。`daemon` 是一个可以将任何正在运行的进程转为后台运行或进行监控的工具，如果进程崩溃，它会自动重启该进程。`daemon` 在 ports 树中被广泛使用，并且可以在基础系统中得到更多应用。

本季度添加了对 `long_opts` 的支持，并对代码库进行了初步重构，为进一步的改进做好准备。目前没有功能性变化，但未来会有更多改进。如果您遇到意外的 bug，请直接联系或者在 Libera IRC 的 `#freebsd-dev` 频道中与我们联系。
...
kinst 是由 christos@ 和 markj@ 创建的一个全新的 DTrace 提供程序，它允许对内核函数中的任意指令进行跟踪。kinst 已被加入到 FreeBSD 14.0 的基础系统中。
...
cloud-init 是在云中配置服务器的标准方式。不幸的是，除 Linux 外的操作系统对 cloud-init 的支持较差，而 FreeBSD 上缺乏 cloud-init 支持也阻碍了希望将 FreeBSD 作为 Tier 1 平台提供服务的云服务提供商。为了解决这一问题，本项目旨在使 FreeBSD 的 cloud-init 支持与 Linux 保持一致，更广泛的计划是使所有 BSD 系统均能获得该支持。
...
* 将各个 OpenStack 组件转换为 FreeBSD 端口
...
同时，请在您所在的本地用户组中宣传这一工作，我们始终需要更多志愿者。

#### FreeBSD Handbook 工作组
...
如果您愿意参与，请查阅[官方翻译指南](https://docs.freebsd.org/en/books/fdp-primer/translations/)。我们非常需要您在以下方面提供翻译帮助：

- 网站页面（较简单）
- Handbook 的章节（目前仅有 X11 章节正在进行中）
- 文章
...
涉及 Ports Collection 的变更，无论是涉及整个端口树的大范围改动，还是单个端口的调整。
...
FreshPorts 和 FreshSource 已对 FreeBSD 的提交进行了长达 20 年的跟踪报道。它们覆盖所有提交，不仅仅是端口相关的。

FreshPorts 跟踪提交情况，并从各个端口的 Makefile 中提取数据，以创建一个对端口维护者和使用者都非常有用的信息数据库。

例如，[https://www.freshports.org/security/acme.sh/](https://www.freshports.org/security/acme.sh/) 展示了 [security/acme.sh](https://cgit.freebsd.org/ports/tree/security/acme.sh/) 端口的历史记录，可以追溯到该端口于 2017 年 5 月创建时。同时，还提供了依赖关系、风味、配置选项和可用软件包等信息。这一切对端口的用户和开发者都非常有用。

#### SQL 注入攻击

三月份时，发现了一起 SQL 注入攻击，并对网站进行了修补。我们通过 Twitter 账号、状态页面以及网站每一页顶部发布了通知。攻击的直接入口已被关闭，并很快完成修补。随后，在网站的各个部分增加了额外的预防性补丁。所有已知的问题均已修复。用户已收到通知，并被建议更改密码。
...
FreshPorts 已运行超过 22 年，未来必须由其他人接手。我希望现在就开始这个交接过程。FreshPorts 涉及多个方面：
...
与应用程序和操作系统相比，数据库变化不大且维护工作较少。网站基本上可以自动运行。偶尔，FreeBSD 端口基础设施的某些变动会导致部分功能出现问题或需要修改，但通常不紧急。这并不会占用太多时间，并且其中包含了大量的学习机会。虽然 FreshPorts 不是一个复杂的应用，但它也绝非易事。
...
截至本次报告，[graphics/drm-kmod](https://cgit.freebsd.org/ports/tree/graphics/drm-kmod/) 元端口仍然在 FreeBSD 13.1 及更高版本上安装来自 Linux 5.10（发布于 2020 年 12 月 13 日）的 DRM 驱动。该版本的驱动缺乏对新款 GPU 的支持，尤其是不支持 Intel 第 12 代 Alder Lake GPU。过去几个月，我们一直在努力更新 DRM 驱动，以支持更新的 AMD 和 Intel GPU。
...
- 创建了 `5.15-lts` 分支，以回溯合并 Linux 5.15.x 的所有补丁修复。该工作现已在 `drm-515-kmod` 端口中提供。  
...
FreeBSD 上的 KDE 项目负责在 FreeBSD 端口树中打包 CMake、Qt 以及来自 KDE 社区的软件。这些软件包括完整的 KDE Plasma 桌面环境（支持 X11 和 Wayland），以及数百款可在任何 FreeBSD 机器上运行的应用程序。  

KDE 团队（kde@）是 desktop@ 和 x11@ 组的一部分，负责构建软件栈，使 FreeBSD 成为一个美观、可用的日常桌面系统。以下内容主要涉及 KDE 相关端口，同时也包含对整个桌面环境至关重要的内容。  

---

#### 基础设施  

- Qt5 端口已更新至 KDE 补丁集合 5.15.8。  
- Qt6 端口（KDE 目前尚未使用，但许多端口支持 Qt6 并提供 Qt6 版本）已更新至 6.4.2，并新增了 Qt6 WebEngine 的 Python 绑定。  
- CMake 端口已更新至 3.25.1，并修复了 FreeBSD 软件包的 CPack 生成器。  
...
KDE Gear 每季度发布一次，KDE Plasma 每月更新一次，KDE Frameworks 也每月发布新版本。这些较大规模的更新会在上游发布后不久合入端口树，因此不单独列出。  
...
- 新增 [devel/ktextaddons](https://cgit.freebsd.org/ports/tree/devel/ktextaddons/) 端口。该软件是 KDE PIM 套件的一部分，未来可能成为新的 KDE Framework 组件。  

---

#### 相关端口  

- [audio/amarok](https://cgit.freebsd.org/ports/tree/audio/amarok/)（2000 年代初最受欢迎的 KDE 音频播放器之一）已在端口树中标记为“已废弃”，因其上游已停止维护。  
...
- [graphics/quickqanava](https://cgit.freebsd.org/ports/tree/graphics/quickqanava/)（图形可视化库）已正式发布，并在端口树中进行了更新。  
...
因此，本季度我用 Rust 重新编写了该工具。新版本在相同的种子值下，与原始工具的行为保持字节级兼容。但未来版本可能会打破向后兼容性，以添加 `fspacectl` 和 `copy_file_range` 等新功能。目前，新版本已进入端口树，未来我将移除原始版本。
