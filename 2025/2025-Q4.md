...
团队管理了 15.0-RELEASE，并于 12 月完成了正式的 RELEASE 构建与发布公告；这是来自新 **stable/15** 分支的首个发布版本。作为其中的一部分，大量工作用于建立一套系统，以便为 15.0-RELEASE 基本系统安全地构建、签名和分发包仓库。
...
Alpha-Omega 的使命是推动关键开源项目和生态系统实现可持续的安全改进。在 2024 年与 FreeBSD 基金会成功合作——审计 bhyve 虚拟机监控器和 Capsicum 沙箱框架——之后，Alpha-Omega 再次选择了 FreeBSD，这次开展 Alpha Omega Beach Cleaning 项目。这项新资助旨在整体提升 FreeBSD 基本系统中第三方软件的安全性和维护状况。FreeBSD 基金会获得了该资助，并负责管理和执行该项目。
...
目前收集到的反馈均为积极反馈，其中包括建议将该工具打包进 FreeBSD ports。然而，目前看来，将该工具重写为适合纳入基本系统的形式（例如使用 Lua）更为合适。
...
软件物料清单（SBOM）项目自 5 月以来一直在进行，其目标是提供必要的工具，以便从 FreeBSD Ports 和基本系统生成 SBOM。

2025Q4 的一项主要进展是将 spdxtool 上游合入 pkgconf。该上游代码确保 pkgconf 工具在下一个版本中具备符合 SPDX Lite 3.0.1 配置文件的 SBOM 创建工具。

另一项重要工作是收集构成 FreeBSD 基本系统一部分的应用程序信息。这些应用程序主要位于 FreeBSD git 仓库中的 usr.bin、usr.sbin、sbin 和 bin 目录。FreeBSD Alpha Omega Beach Cleaning 项目在此过程中发挥了关键作用，因为它收集了有关第三方库和应用程序的信息，我也为此做出了贡献。现在已有 Lua 脚本和一个文件，可以生成 pkgconf 的 spdxtool 所需的文件，并可导出为 SPDX JSON-LD 格式。
...
该项目于 2024 年第三季度启动，由 Sovereign Tech Agency 委托，预算为 745,000 美元，支出截止至 2025 年底。主要目标是改善基本系统、Ports 和软件包的安全工具，更新项目基础设施以加快开发速度，增强构建安全性，并使新开发者更易上手。
...
我们进行了修改（目前审查中），将基本系统和 Ports 树的单个来源数据整合为统一的高级视图。我们还创建了工具，可扫描 FreeBSD 源代码树并生成涵盖整个软件栈的 SBOM 报告。
