...
FreeBSD 基金会是一家 501(c)(3) 非营利组织，致力于通过技术、非技术支持推动 FreeBSD 的发展。基金会完全依靠捐款资助，支持软件开发、基础设施、安全与协作工作；组织活动和开发者峰会；提供教育资源；并在法律事务中代表 FreeBSD 项目。
...
基金会持有 FreeBSD 商标，并有责任对其进行保护。我们还为核心团队提供法律支持，以调查出现的相关问题。
...
全局数据库文件包含了为该项目收集的信息，并与由德国 Sovereign Tech Agency 资助的 SBOM 计划协作完成。其结构在过去几个月中也已简化，但仍保持 YAML 格式。它仍然以 [database.yml](https://github.com/FreeBSDFoundation/alpha-omega-beach-cleaning/blob/main/database.yml) 的形式提供。

仓库中的 Go 程序 aobc-generate 更名为 aobc-tool。除之前的交付成果外，现在它还能够生成一组 SBOM 文件。这通过 pkg-config 格式的中间文件完成，然后借助 pkgconf 项目的 bomtool 程序将其转换为 SPDX：
...
该计划已于 11 月向源代码管理团队进行了介绍。他们的意见和反馈将在项目的最后阶段予以考虑。
...
目前已有一个测试数据库和一个针对 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 的拉取请求。该测试数据库可以从 VuXML 更新并转换为 OSV JSON 格式。已完成用于更新、创建供 pkg 使用的合并数据库文件的所需工具。还支持导出为 Commonmark 格式，其在 GitHub 上可正常渲染。
...
自 5 月以来，软件物料清单（SBOM）项目一直在进行着，其目标是提供必要的工具，以便从 FreeBSD Ports 和基本系统生成 SBOM。

2025Q4 的一项主要进展是将 spdxtool 合入了上游的 pkgconf。该回溯代码可确保在下一个版本中，pkgconf 工具具备符合 SPDX Lite 3.0.1 配置文件的 SBOM 创建工具。

另一项重要工作是收集构成 FreeBSD 基本系统一部分的应用程序信息。这些应用程序主要位于 FreeBSD git 仓库中的目录 `usr.bin`、`usr.sbin`、`sbin` 和 `bin`。FreeBSD Alpha Omega 海滩清理项目在此过程中发挥了关键作用，因为它收集了有关第三方库和应用程序的信息，我也为此做出了贡献。现在有 Lua 脚本和一个文件，可以生成 pkgconf 的 spdxtool 所需的文件，亦可导出为 SPDX JSON-LD 格式。
...
因此，作为其正常操作的一部分，bhyve 为其 VM 客户端模拟 x86 CPUID 指令。目前，大部分来自主机 CPU 的 CPUID 信息会传递给 VM 客户端，同时屏蔽掉部分特性，并且还报告了关于虚拟机管理程序、其能力和配置的综合信息。目前在 bhyve 中，这些均尚不可显式配置。
...
控制客户端 VM 的 CPUID 信息允许操作员限制 VM 使用所有可能运行的 VM 主机共享的 CPU 特性子集，这也可以基于抽象的 x86 架构级别，而非物理的 CPU 型号。
...
截至 2025 年 12 月底，所有五个工作包均已完成，项目已结束。撰写本文时（12 月中旬），部分内容仍在审查，这些内容已移交给基金会工作人员进行适当落地。
...
我们进行了修改（目前正在审查），将基本系统和 Ports 树的单个来源数据整合为统一的高级视图。我们还创建了工具，可扫描 FreeBSD 源代码树并生成涵盖整个软件栈的 SBOM 报告。
...
Parthenope 起初是个人项目，用于回答一个简单问题：“在我的笔记本上安装 FreeBSD 时，我希望有哪些功能？”此后，它发展为开源工具，任何人都可以使用、适配和扩展以满足自己的需求。
...
该后端目前功能已完善，可使用单个 vCPU 启动 FreeBSD 客户端。用于将 QEMU 与 `libvmmapi` 接口对接的核心基础设施已建立。关键点是，现在成功使用内核的 `vmm` 实现了中断控制器（8259 PIC、IOAPIC、LAPIC），而非用户空间模拟。
...
Sylve 是现代化的 FreeBSD 统一系统管理平台，灵感来自 Proxmox。它提供集成的 Web 界面，用于管理虚拟机（通过 Bhyve）、Jails、ZFS 存储、网络和防火墙。
...
自 2024 年第一季度以来，我一直在进行音频栈相关工作。下面是之前的状态报告：
...
我从事 GENEVE 隧道的实现工作已经三个月了，目前处于审查阶段。GENEVE 在 L3 (IP/UDP) 下层之上为租户系统创建通用网络虚拟化隧道接口，可使用 GENEVE 协议提供第 2 层（以太网）或第 3 层服务。
...
* 编写 [geneve(4)](https://man.freebsd.org/cgi/man.cgi?query=geneve&sektion=4&format=html) 手册页，再更新 [ifconfig(8)](https://man.freebsd.org/cgi/man.cgi?query=ifconfig&sektion=8&format=html) 来包含 geneve 参数。
* 编写测试。
...
此次审查较大，因为我在提交前已实现其他平台已有的功能。你可以通过在 Phabricator 上审查，提供反馈来加快进程。
...
我一直在为内核添加 Rust 支持，并尝试使用 Rust 编写设备驱动。Rust 可以通过类型系统强制执行关于指针使用的规则，而这些规则在 C 中通常是隐含的，造成了开发者额外的心智负担。Rust 虽然不能防止所有错误，但它可以帮助开发者减少这些负担，更专注于实现核心功能。

Rust KPI 当前仍在开发中，尚不稳定，但它们不依赖于更广泛的 Rust 生态系统或不稳定语言特性/API。这意味着它们最终应能提供与 C KPI 等效的稳定性，升级到新 Rust 工具链不应引入破坏性变更。它也提供了与 C 同等的透明度，明确何时发生分配，因为使用的是非分配子集的 [Rust 标准库](https://doc.rust-lang.org/core/)。不同内核子系统可能需要 Rust 封装，但这些可以基于 rust KPI 仓库提供的基础功能构建。
...
本季度，对 XHCI 调试主机端接口进行了大量调试工作。设计了一块定制板，支持 Type-C 的两种调试模式：XHCI 调试和 Type-C 调试配件模式（DAM）。DAM 为 Type-C 接口提供替代模式，在部分设备（如 2025 Framework 笔记本和桌面机）上，该模式可访问 SOC UART 进行调试。

为了完成完整的 XHCI Type-C 适配器和更完整的 DAM 适配器，仍需进一步改进硬件。

加载器部分的实现得到了创建评审，提取了若干更改，已合并入源码树。
...
* 手册在 FreeBSD 2025 萨格勒布开发峰会上进行了展示，幻灯片见 [此处](https://docs.google.com/presentation/d/1rnw5fU8KyUCGNqR6gkM5IBdyx8cVVdqigCGmrrd%E2%80%94%E2%80%8Ba4)。
...
手册主观上避免使用图片，尽量减少非纯文本元素，旨在提高与辅助技术的兼容性。欢迎提供建议和新思路，可将反馈发送至 FreeBSD 可访问性邮件列表，以便公开讨论和跟踪。
...
* 剩余文档的审查与更新已完成，47 个文档（书籍和文章）均已在 [俄语文档站点](https://docs.freebsd.org/ru/) 得到同步。
...
Plasma/Wayland 的支持得到了改进，可作为日常驱动替代 Plasma/X11 使用。
...
它们是旧 ports 的直接替代：虽然软件包名为 “openvox”，服务名称暂时保持不变。要切换，只需安装它们，并在 pkg 提示移除旧软件包并安装新软件包时选择是。无需其他操作：之前使用 Puppet 的模块应继续在 OpenVox 上工作。

今年 Puppet 7 已达到生命周期终点，对应的 port 已从 FreeBSD ports 中删除。Puppet 7 是最后一个能在 C 版本与 Ruby 版本 facter 之间选择的版本，因此 C 版本 port ([sysutils/facter](https://cgit.freebsd.org/ports/tree/sysutils/facter/)) 也被删除。由于 Puppet 8 的旧 port 不再更新，将很快被弃用，并遵从同样的命运。
