...
由于同步缓存代码的提交滞后，该项目的开发进度放缓，因为该项目依赖于这部分代码。
...
构建过程现在会创建四个不同版本的库，其中包括对 TVision、Qt 的支持，或两者，或者都不支持。我已经从现有的 Ports 创建了一些初始软件包，并且仅使用 libh 的工具安装了这些 libh 软件包，包括在软件包数据库中注册所有文件，记录它们的校验和等。对磁盘编辑器的补丁已经提交，包括写入 fdisk 部分更改的功能，并初步支持磁盘标签编辑器。我们很快就会有一位新的提交者。
...
在本月的 PowerPC 移植工作中，取得了一些重要进展。Mark Peek 成功地在 FreeBSD 上交叉编译了 FreeBSD/PowerPC 内核，并使其在 PSIM 模拟器（现在在 **/usr/ports/emulators/psim-freebsd**）下启动。我也成功地使 FreeBSD 引导加载程序能够使用 Apple Macintosh 硬件上的 OpenFirmware 加载和执行内核。Mark 现在正在完成一些启动代码和 `pmap` 代码，而我则利用模拟器来处理一些中断和设备问题。
