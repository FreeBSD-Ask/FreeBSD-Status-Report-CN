...
FreeBSD C99 和 POSIX 合规性项目的工作进展顺利。自上次状态报告以来，已添加了两个新头文件 `<stdint.h>` 和 `<inttypes.h>`，实现了多个新函数 **atoll(3)**、**imaxabs(3)**、**imaxdiv(3)**、**llabs(3)**、**lldiv(3)**、**strerror_r(3)**、**strtoimax(3)** 和 **strtoumax(3)**，并对 **assert(3)** 和 **printf(3)** 进行了更改以支持 C99。此外，还有更多的 **printf(3)** 更改正在进行中，以支持剩余的 C99 和 POSIX 要求。还对 POSIX 实用程序兼容性进行了研究，并从中得出了任务列表。
