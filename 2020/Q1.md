...
FreeBSD CI 团队维护着 FreeBSD 项目的持续集成系统及相关任务。CI 系统定期检查提交的更改是否能够成功构建，并执行各种测试和结果分析。构建任务的构件会存档在构件服务器中，以便进行进一步的测试和调试需求。CI 团队成员会检查失败的构建和不稳定的测试，并与该领域的专家合作，修复代码或调整测试基础设施。这些工作的详细信息可以在 [每周 CI 报告](https://hackmd.io/@FreeBSD-CI) 中找到。
...
在我们在上个季度报告中请求帮助之后，i386 wine Port 已由 salvadore 接管，并立即开始解决现有的 bug 并改进这些 Port。大部分工作已经完成，我们已于 3 月开始提交首批更新。由于这项工作比最初预期的花费更多时间，因此我们也将在此过程中更新 i386-wine-devel Port，以便需要更近期版本的用户可以轻松地从 Ports（或二进制软件包）中获取。另一方面，我们计划在 i386-wine-devel 更新完成后，回溯将这些改进应用于 i386-wine，之后才更新该 Port，以确保 i386-wine 始终保持稳定版本。
