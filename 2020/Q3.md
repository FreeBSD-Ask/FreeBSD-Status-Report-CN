...
此代码是在 2020 年谷歌编程之夏（GSoC）的指导下完成的，由 Ryan Stone（rstone@）提供指导。FreeBSD 的 eBPF 实现仍在进行中，目前 FreeBSD 尚不支持 eBPF。eBPF 的基础实现是 2018 年编程之夏项目的一部分，并且仍在开发中。该项目基于该实现，因此只有当 FreeBSD 支持 eBPF 时，XDP 实现才会合并到 FreeBSD 源代码中。
