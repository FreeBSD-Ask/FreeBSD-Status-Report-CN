...
在第四季度，我们增加了 4 名新的提交者，收回了 4 个提交位，并且有一名提交者重新参与了 Ports 工作。
...
我们很高兴宣布，FreeBSD Haskell 团队将 Haskell 平台更新为 2011.4.0.0，并且将 GHC 更新为 7.0.4，更新内容提交到了 FreeBSD Haskell Ports 仓库中。我们还增加了若干新的 Haskell Ports，当前总数超过了 300 个。其中一些新添加的 Ports 包括 Yesod、Happstack（Haskell 中流行的 Web 开发框架）、ThreadScope（用于并行 Haskell 程序的图形分析工具）。
...
许多 Perl 模块在 Ports 中自带测试用例。这个项目的目标是确保所有这些测试都能通过。向 Ports tinderbox 添加了补丁，以便安装与测试相关的依赖项，许多 Ports 现在包含了 TEST_DEPENDS。希望提供帮助的用户可以使用补丁启用测试功能。所有 `p5-*` Ports 都构建了并尝试运行测试。目前，约有 61% 的 Perl Ports 测试通过。许多 Ports 更新了，包含了缺失的依赖项或进行了其他修改，使测试能够通过。长期目标包括为测试 Ports 提供一个更通用的框架，并在更新 Ports 时执行自动化测试。
