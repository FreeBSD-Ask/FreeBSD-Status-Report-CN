...
AFS 是一个分布式网络文件系统，起源于卡内基梅隆大学的 Andrew 项目。自 FreeBSD 4.X 版本以来，OpenAFS 客户端在 FreeBSD 上的实现一直不太实用。先前的工作将 OpenAFS 客户端移植到 9.0-CURRENT 上，尽管仍有一些粗糙的地方。自上次报告以来，我们修复了几个影响可用性的 bug，并预计即将发布的 1.6.0 版本将适用于常规客户端工作负载（尽管不适合重负载）。因此，我们提交了包含在 Ports 中的打包（PR ports/152467）。
...
## [Ports](https://www.freebsd.org/status/report-2011-01-2011-03.html#Ports)
...
在过去的几个月里，我们继续在 HTPC 相关的 Port 上工作，改进了 lirc 和 **multimedia/webcamd** 遥控支持。最后一个缺失的主要 HTPC 应用程序 VDR（视频磁盘录音机）终于作为 **multimedia/vdr** 包含在 portstree 中，并提供了 17 个 vdr 插件 Port。
...
### [FreeBSD Haskell Ports](https://www.freebsd.org/status/report-2011-01-2011-03.html#FreeBSD-Haskell-Ports)
...
我们很高兴宣布，FreeBSD Haskell 团队将 GHC 更新到 7.0.3，并将所有现有的 Haskell Ports 更新到最新的稳定版本，并添加了新 Port。FreeBSD 仓库中的 Haskell Ports 总数已超过 200 个。当前这些 Port 仍在等待提交。它们暂时可以通过 [FreeBSD Haskell Ports 仓库](https://github.com/freebsd-haskell/freebsd-haskell) 获取。任何希望尽早访问这些 Port 的用户，请参考 [FreeBSD Haskell Ports 测试请求](http://www.haskell.org/pipermail/freebsd-haskell/2011-April/000278.html)。
...
我们的演讲名单已经确定，日程安排几乎完成。现在还有时间加入进行中的工作会话。
...
FreeBSD 很荣幸第七次参加 2011 谷歌编程之夏。我们于 4 月 25 日星期一接受了 17 个提案，整体质量非常优秀。可以在谷歌编程之夏网站找到所有被接受提案的完整列表。我们期待在夏季与这些学生共同工作。
