...
我们很高兴宣布，FreeBSD Haskell 团队将 GHC 更新到 7.0.3，并将所有现有的 Haskell Ports 更新到最新的稳定版本，并添加了新 Port。FreeBSD 仓库中的 Haskell Ports 总数超过 200 个。当前这些 Port 仍在等待提交。它们暂时可以通过 [FreeBSD Haskell Ports 仓库](https://github.com/freebsd-haskell/freebsd-haskell) 获取。任何希望尽早访问这些 Port 的用户，请参考 [FreeBSD Haskell Ports 测试请求](http://www.haskell.org/pipermail/freebsd-haskell/2011-April/000278.html)。
...
完成了 95%，待最终 -exp 测试运行，并切换。相关邮件发送给相关邮件列表。这将适用于 8.3/9.0。
