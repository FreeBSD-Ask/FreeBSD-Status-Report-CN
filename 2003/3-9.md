...
几个月前，我开始尝试以更用户友好的方式呈现 [bento 构建集群](http://bento.freebsd.org/) 中包含的信息；即按错误类型、维护者等进行浏览。最早添加的功能是一段代码，尝试将 Ports PR 分类为“现有 Port”（在分配最可能的类别和 Port 名称后）、“新 Port”、“框架”（例如 `bsd.port.mk` 变更）、“未知”。我们还向报告中添加了关于 Port PR 的多个列。

这一功能的初衷是让 Ports 维护者的工作变得更加轻松；然而，“通用”报告对任何想要查看特定 Port 是否在他们的架构和操作系统组合中正常工作的人也非常有用。在下载之前，大家可以查看特定 Port 的概览，来了解其兼容性。对此感兴趣的用户可以从 [Port 概览](http://lonesome.dyndns.org:4802/bento/errorlogs/portoverview.py) 开始。
...
自 5.1 之后 ABI 没有发生变化，预计变化不会太大。这意味着我们可以考虑成为一级平台。这也意味着我们需要 **gdb(1)** 的支持。相关工作已经开始，但这条路依然崎岖漫长。内核的稳定性也有了显著提升，目前通常只剩下一个内核 panic：VM 在无缺页入口处发生缺页。这个问题将通过期待已久的 PMAP 改进来解决（见下文）。

FreeBSD 5.2 主要的工作将是“精益求精”，收拾遗留的收尾工作。这一计划的调整得益于发布日程的推迟。5.2 版本将不会是 `-stable` 分支的起点，它已被推迟到 5.3 版本。因此，我们将利用额外的时间为 5.3 做好准备。
...
FreeBSD Port 已更新至 KDE 3.1.4，这是另一个包含 bug 修复和安全修复的版本。通过此更新，QT Port 已更新至 3.2 版本。这两个版本将包含在 FreeBSD 4.9 中。大量工作已投入到在 FreeBSD-CURRENT 中修复 KDE，特别是在去除 gcc `-pthread` 选项之后。自动构建来自 KDE CVS 的软件包继续确保并提高即将发布的 KDE 3.2 的质量。
...
自上次报告以来，已经完成了大量工作：网站已完全重新设计（请参见新网址），实现了带有文本或图形模式控制台消息的控制台设计，开发了一个兼容层以编译 Linux fbdev 驱动程序，对原始驱动程序进行或多或少的修改（实验性）。
