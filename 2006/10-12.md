...
FreeSBIE 正接近 2.0-RELEASE。第一个发布候选版本证明已足够好，但可能会发布第二个候选版本。一位外部开发者正在努力将 BSDInstaller 集成到 FreeSBIE 2.0 中，这可能会稍微延迟发布日期。版本说明已编写，并需要更新当前的软件包列表。已添加一个可以开关 Tor+Privoxy 的脚本，并对其使用进行了文档记录。2.0-RELEASE 即将发布，预计在 1 月底前，但这也取决于 FreeBSD 6.2-RELEASE 的发布日期。
...
在过去的一个季度中，进行了许多改进，目前报告称 3 个架构（i386、amd64 和 arm）已可以正常工作，项目可以认为功能已完备。
