...
这是一个更新，说明 TS-7200 已在最近的 -current 版本上成功构建并运行。

我一直在努力让 FreeBSD/arm 在 TS-7200 上运行。目前，主板已能够启动，并且以太网部分能够工作（尽管存在一些无法解释的数据包丢失问题）。我可以从 FreeBSD/i386 机器上进行网络启动，并且能够在 CF 上挂载 msdosfs。
