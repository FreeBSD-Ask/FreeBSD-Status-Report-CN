...
FreeBSD 的项目和志愿者想法列表进展顺利。多个项目已经由志愿者接手，并成功合并到项目中。其他项目正在审查或进行。
...
Libpkg 是使用 libarchive 提取软件包文件的软件包管理库。它可以下载、安装并列出已安装的软件包。目前已经开始实施基本系统中的软件包工具。已实现大部分 pkg_info，也已经开始实现 pkg_add。
...
自 2003 年以来，我推出了（现在相当广泛使用的）FreeBSD Update 和 Portsnap 工具，但由于时间限制，很少有机会对其做改进或添加请求的功能。因此，在 3 月 30 日，我向 freebsd-hackers、freebsd-security 和 freebsd-announce 列表发送了电子邮件，宣布我正在寻求资金，以便让我能够在夏季全职从事这些工作并担任 FreeBSD 安全官。如果一些支票按预期到达，我已经达到了捐赠目标，并将在 5 月初开始工作。
