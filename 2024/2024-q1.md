...
例如，你可以通过 [https://www.freshports.org/security/acme.sh/#history](https://www.freshports.org/security/acme.sh/#history) 查看 [security/acme.sh](https://cgit.freebsd.org/ports/tree/security/acme.sh/) Ports 的历史记录，从 2017 年 5 月开始。还可以查看依赖关系、flavors、配置选项和可用的软件包。这些信息对 Ports 的用户和开发者都非常有用。

#### 通知：新软件包可用

FreshPorts 的原始功能之一是 Ports 更新通知。你可以创建一个 Ports 列表，并接收关于这些 Ports 的通知。此新功能还可以在某个 Ports 有新软件包可用时通知你。应用场景：已知的安全漏洞已被修复。FreshPorts 会告诉你该 Ports 已经修复，你可以等待软件包的发布。这个新功能会在软件包可用时通知你。
