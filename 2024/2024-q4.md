...
涉及多个类别的项目，从内核和用户空间到 Ports/外部项目。
...
* 工作包 A：技术债务减少。基金会与源代码管理团队合作，委托并部署了多个仪表板，用于概览 FreeBSD 项目的错误积压情况。这些仪表板由我们的项目合作伙伴 Bitergia 使用开源工具 GrimoireLab 创建，并按照团队的规格设计。基金会工作人员将这些仪表板托管在 FreeBSD 部署上，访问地址为 [https://grimoire.freebsd.org/](https://grimoire.freebsd.org/)。可以在 [https://github.com/freebsd/grimoire](https://github.com/freebsd/grimoire) 上找到更多有关仪表板的信息。  
...
* 基金会 [启动了该项目](https://freebsdfoundation.org/blog/why-laptop-support-why-now-freebsds-strategic-move-toward-broader-adoption/)，创建了[公开路线图](https://github.com/FreeBSDFoundation/proj-laptop)，并分配了承包商到相关工作流。12 月是首次开发迭代，覆盖了以下内容：
  ...
  * [引入戴尔的相机代码贡献](https://github.com/FreeBSDFoundation/proj-laptop/issues/39)
...
这使得选项 ALLOW_BASE_SHLIBS 得以消除。它还改进了 Port 包与 PkgBase 包之间的集成，因为现在跟踪了 Port 包对 PkgBase 包的共享库依赖，而不是忽略这些依赖。最后，这一变化显著简化了 pkg 的代码库并提高了可移植性。此修改已在 [https://github.com/freebsd/pkg/pull/2386](https://github.com/freebsd/pkg/pull/2386) 中实现，但尚未包含在 pkg 发布版本中。
...
* 尝试修补自动化 [snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html) 插拔。
* 调查 SOF/DMIC 支持。
...
驱动程序 [rtw89(4)](https://man.freebsd.org/cgi/man.cgi?query=rtw89&sektion=4&format=html) 已经移植，位于源代码中有一段时间了，并已成功连接到构建中。感谢社区的努力，之前妨碍其工作的两个 bug 已被解决。
...
* 为 wtap 引入一个新的命令 netlink ，以实现帧注入，这是 WiFi 模糊测试的关键。
...
来自微软的 Souradeep Chakrabarti 添加了一个特性，用于在 Hyper-V 和 Azure 上使用超调用进行 TLB 清除。
...
可在 amd64（Intel 和 AMD）和 arm64（Graviton）EC2 实例上运行 FreeBSD。

在过去一个季度中，ZFS AMI 的首次启动性能有了显著改善，例如，15.0 `base` AMI 在 amd64 上的启动时间从约 22 秒减少到约 11 秒。启动性能的图表现在会自动生成、发布；典型的启动时间为 `base` 和 `small` AMI 约 9-12 秒，而 `cloud-init` AMI 为 14-18 秒。
...
文档工程负责处理与 FreeBSD 文档项目相关的一些元项目问题；有关更多信息，请参阅 [FreeBSD Doceng 团队章程](https://www.freebsd.org/internal/doceng/)。
...
FreeBSD 提供 Ports，为用户和管理员提供了一个简单的应用程序安装方式。Ports 提供了成千上万的 Port；Port 配置是其核心功能之一。在构建和安装之前，可以配置 Port。命令 `make config` 使用文本用户界面（TUI）以交互方式设置 Port 选项。
...
Erlang 运行时系统，通常称为 BEAM，提供了一个运行时环境，许多编程语言和应用程序都使用它，可以在 FreeBSD  Ports 中找到这些应用程序。
