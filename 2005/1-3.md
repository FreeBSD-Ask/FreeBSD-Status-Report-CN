...
我已设置测试系统以测量 FreeBSD 5.3 和当前版本的中断延迟。到目前为止，我已测量了基于 300MHz 嵌入式 Cyrix 的单板计算机的基准延迟。我尝试了多种不同的策略来优化中断路径。这些策略大多能改善从中断服务开始到驱动程序 ISR 的时间。这些改进约占该单板计算机处理时间的 1-2%，但对更快的机器来说影响不大。然而，从中断应该发生的时间，到 FreeBSD 开始处理中断的时间，是这些测量中的主导因素。尽管这些是快速的中断处理程序（因此调度器不参与其中），我经常看到平均延迟为 18 微秒，并且有较大的波动（标准差约为 5 微秒）。
