...
经过漫长、令人疲惫但极富有成效的 2005 年第三季度，FreeBSD 6.0 已发布。为了让这个版本取得成功，许多活动被推到了后台。
...
`nsswitch` 子系统被扩展以支持新的源（服务、协议、`rpc`、`openssh` 和 GT4）。Globus Grid Toolkit 4 补丁（为 GT4 添加 `nsswitch` 支持）的测试仍有待完成。为了使 `nsswitch` 支持缓存，缓存守护进程在缓存库的基础上实现，该缓存库也是在 SoC 期间开发的。当前版本的守护进程使用简单的 `nscd` 样式配置文件，并且似乎稳定。为了完成 SoC 项目，开发了启用了进程内缓存的 `libc` 实验版本，不久将进行基准测试。
