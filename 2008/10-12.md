...
随着 FreeBSD 7.1 发布，Port 更新即将到来，但迫不及待的人可以按照 BSD# 项目页面上提供的说明，立即下载并合并 BSD# Port 到他们的 FreeBSD 树中。
...
在最近的 Strasbourg DevSummit 上，参与者花了半天时间处理当前的“推荐 PR”列表。这个列表按日期划分为不同部分，每个小组分配一个部分处理。结果，不仅提交了不少修复、关闭了相应 PR，而且开发人员也了解了 BugBusting 团队正在做的分类工作（见下文）。我们希望在未来继续推动这一势头。此外，讨论了许多改进报告页面的新想法。
...
Port 数量跃升至超过 19,600。PR 数量在冻结/临时冻结周期期间有所增加，但现在回落到大约 900 的常规数量。
...
自上次状态报告以来，7.1-RELEASE（2009 年 1 月 5 日）和 6.4-RELEASE（2008 年 11 月 28 日）都已发布。从 6.4-RELEASE 开始，提供了一个新的 DVD ISO 映像，名为 “dvd1”，适用于 amd64/i386。这个映像包含了 CDROM 光盘上的所有内容。因此，“dvd1”可以用于执行包括基本软件包集的完整安装，它包含了所有支持语言的文档，并且可以用于启动进入“基于 live CD 的文件系统”和系统恢复模式。6.4-RELEASE 是 6.X 分支的最后一个版本，目前没有计划发布其他 6.X 版本，因为大多数开发人员已集中精力在 `8-CURRENT` 和 7.X 上。
