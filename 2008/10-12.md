...
由于时间紧张，Mono 在 FreeBSD ports 树中已经停滞在 1.2.5 版本超过一年。然而，现在情况有所变化，BSD## 团队自豪地宣布，Mono ports 即将更新到 2.0.1 版本。依赖 Mono 的 ports 也将同时更新到最新可用版本。
...
此外，还有一个新的面向 PR 提交者的报告，总结了 [最常报告的问题](http://people.freebsd.org/~linimon/studies/prs/well_known_prs.html)。这些问题之所以长期存在，是因为它们很难解决。在提交 PR 之前，您可能想先查看此列表。
...
CPU 层（内核启动、TLB 处理）源自现有的 E500 支持。最终，代码将进行重构，以便共享处理器变体之间的通用逻辑，仅提供最低级别的例程。还需要为芯片上集成的外设编写多个驱动程序（以太网、PCI/PCI-Express、加密引擎、SATA、I2C、SPI、GPIO 等）。
...
4. 目前尚未支持主机到 PCI-Express 或主机到 PCI-X 桥接器，至少后者是因为无法访问此类机器。将对现有的 schizo(4) 添加对 XMITS PCI-X 桥接器的支持应该相对直接，然而 PCI-Express 需要一个新驱动程序，并可能需要一些额外的调整。
...
我们目前正在寻找更多来自 [BSDCan](http://www.bsdcan.org/)、[EuroBSDCon](http://www.eurobsdcon.org/)、[AsiaBSDCon](http://www.asiabsdcon.org/) 等会议的视频上传到频道。如果您希望帮助为您的语言生成字幕，或者如果您有来自上述 BSD 会议的旧视频内容，请告诉我们。

#### 待办事项

1. 为所有技术讲座添加各种语言的字幕。
2. 寻找更多来自以往会议的视频进行上传。
3. 音频后期处理。如果有人有去除视频录音中音频伪影的经验，我们希望能与您合作，在上传到 YouTube 之前处理我们手头的原始视频。
