...
由于上述工作，我们已能将总 PR 数量控制在约 5300 个左右（峰值约为 5500）。尽管 PR 提交量最近有所上升，且 port PR 积压比近期趋势略高（由于长时间的冻结/缓冲周期）。然而，最令人鼓舞的是，我们不仅控制了绝对数字，还能够更迅速、更全面地处理传入的 PR。尽管我们还未完全达到理想状态，但这一趋势非常令人鼓舞。
...
一个原型网页应用已为 `http://www.FreeBSD.org` 网站编写，经过认证的用户能添加新的开发创意，或评论和投票已有的创意。该应用是目前用于维护项目创意（为暑期编码学生及其他希望参与 FreeBSD 的人提供）的静态网页的拟替代方案。
