...
在本季度，MATE 1.6 终于导入到 Ports 中，这要感谢 Jeremy Messenger 的努力。MATE 是一个从现已不再维护的 GNOME 2 代码库分叉出来的桌面环境，因此它基本上是 GNOME 2 的替代品。建议希望保留 GNOME 2 作为桌面的用户切换到 MATE，因为 GNOME 2 将在不久的将来由 GNOME 3 替代。这个切换会提前宣布，以便人们有时间转移到 MATE，如果他们还没有这么做的话。完整的基于 MATE 的桌面环境可以通过 `x11/mate` Port 安装，或如果需要最小化安装，可以通过 `x11/mate-base`。
