...
iSCSI 是一种流行的块存储协议。在这个项目下，已实现了一个新的、快速且可靠的基于内核的 iSCSI 启动器（客户端）和目标（服务器）。
...
在 2013 年，FreeBSD 基金会赞助了 Benno Rice 完成一个小项目，旨在改进 UEFI 启动加载程序。最终成果是在 UEFI 项目分支中完成了一个可行的概念验证，但还没有准备好合并到 FreeBSD `head` 中。
