...
在 FreeBSD 世界中，我们度过了又一个非常活跃的三个月，其中包括两次开发者峰会（BSDCam 和 EuroBSDcon），这些将会在单独的状态报告中详细介绍。FreeBSD 在安全性方面继续加大力度，改进了随机数生成的性能和可靠性，并对基本系统中的程序进行了更多隔离。对于开发者来说，正在开发一款新的现代化调试器。此外，FreeBSD 还在通过 Port 支持 Objective-C 和 Ada 语言方面进行显著的现代化，使 FreeBSD 成为开发这两种语言的第一流平台，此外，基本系统中已经具备的 C++11 和 C11 支持也得到了加强。服务器用户将会看到 iSCSI 栈和可扩展性的改进，允许在普通硬件上每秒超过一百万次 I/O 操作，而桌面用户则会看到对新 GPU 的 X 支持和可能的 X 替代方案的改进。
