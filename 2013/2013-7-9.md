...
这些更改已经由多人进行测试，并将在 FreeBSD `head` 中提交，之后将在 FreeBSD 10.0 发布周期结束后合并到 **stable/10**。
...
FreeBSD 的 CSPRNG 还允许某些认为是“高质量”的随机源直接为 **random(4)** 设备提供输入，而无需经过 Yarrow。随着近期有关政府监控和参与选择这些“高质量”源的揭露，人们认为这些源不再可信，因此也必须通过 Yarrow 进行处理。

这个问题在 8 月的剑桥开发者峰会和 9 月的 EuroBSDcon 2013 上进行了不同层次的讨论。

此项工作现已完成，**random(4)** CSPRNG 现在已提升到更为偏执、现代化的标准，针对其熵源采取不信任的策略。还进行了基础设施工作，以便系统管理员更方便地选择熵源。
...
3. 提供某种机制通过 EC2 `user-data` 配置实例。这可能涉及使用 `cloud-init`，或者可能是新的系统。
...
Capsicum 及相关项目（如 Casper、`libnv` 等）的工作进展顺利。`cap_rights_t` 的重构已提交到 FreeBSD `head`，并将包含在 10.0 中。这使我们可以在文件描述符上拥有比以前 64 个权限限制更多的权限，之前的限制几乎已经达到。这一变化不向后兼容，因此将其包含在 10.0 中非常重要。
...
自上次状态更新以来，LLDB 移植已做出多项改进。单元测试失败已排查，并在 LLDB 的错误跟踪器中输入了缺陷。结合 `lldb` 构建机器人，这使得能够迅速识别其他正在进行的开发所引入的新失败。还添加了核心文件支持。

LLDB 快照已导入到 FreeBSD 基本系统，并且从 SVN 修订版 255722 起可用。默认情况下尚未构建，但可以通过在 **src.conf(5)** 中添加 `WITH_LLDB=` 启用。
...
MATE，从现在不再维护的 GNOME 2 代码库分支出来的桌面环境，已经接近完成。
...
这是个谷歌编程之夏 2013 项目，旨在用智能下载管理器服务替代基于 **fetch(1)** 的方法来获取第三方应用程序（Port）的分发文件（如源 tarball）（更多信息请参见链接）。
