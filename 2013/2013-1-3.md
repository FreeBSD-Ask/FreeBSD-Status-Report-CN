...
FreeNAS 8.3.1-RELEASE-p2 将于 4 月的第二周在 Sourceforge 上发布，并应成为基于 FreeBSD 8.X 的最后一个 FreeNAS 发布版本。目前，它是唯一一款提供任何形式 ZFS 加密（由 GELI 提供）的自由开源 NAS 产品。
...
该项目的重点扩展至涵盖新的 iSCSI 启动器。与旧版本相比，它更加可靠、更易用，并且速度略快。它使用与旧版本完全相同的配置文件格式，以便于迁移。
...
本季度为 2 位新提交者授予访问权限，并收回了 1 个提交位。
...
在第一季度，我们新增 4 位提交者，收回了 1 个提交位，并恢复了 1 个提交位。
...
我们一直在创建 BSD 许可的多路径 TCP 实现——这是一组 TCP 扩展，允许通过多个 IP 地址作透明的多路径操作，如实验性 RFC6824 所指定。

我们在 2013 年 3 月 11 日发布了 v0.1 版本，并于 2013 年 4 月 16 日发布了 v0.3 版本。该代码目前视为 alpha 质量。我们正努力将代码推送到 FreeBSD Subversion 仓库项目分支，以便在一个更公开可访问的地方继续开发。作为这次迁移的一部分，我们希望开始发布由 佐藤広生 和 allbsd.org 日常快照基础设施提供的 MPTCP 项目分支的常规快照安装程序 ISO。
...
关键思想是，在每个圆柱组中，紧接着 inode 块后面预留一个小区域，用于存放元数据，特别是间接块和目录内容。新策略是优先将元数据放入元数据区，其他内容则放入紧随其后的块中。
...
ARM 架构越来越普遍，不仅仅在移动和嵌入式领域。最近几个月出现的一个有趣的行业趋势是“ARM 服务器”概念。一些顶级公司（如 Dell、HP）已开始开发这类系统。
...
我们自豪地宣布 FreeBSD Haskell 团队已将现有的 Port 更新为最新的稳定版本。我们还增加了一些新的 Port，使 FreeBSD Ports 树中的 Haskell Port 数量超过 400，涉及许多流行的软件，如 xmonad、git-annex、pandoc 或各种 Web 框架实现。所有这些更新将在即将发布的 8.4-RELEASE 中提供。我们还了解到，Haskell Port 在 DragonFlyBSD 的 dPorts 中也成功使用。

在我们的开发仓库中，增加了一些基于 LLVM 的代码生成的可选支持，使用的是 GHC LLVM 后端。这在 FreeBSD 上也大多可以工作，尽管一些 Ports 仍需修复，因此仍视为实验性。
...
目前，CFT 补丁已提交进行 exp-run 以解决最后的错误。计划是在 FreeBSD 8.4 发布并且 Ports 树重新开放提交后合并。

此外，工作还在推进，将新版本的 MESA 和 OpenGL 以及 xorg-server 的新版本移植到 FreeBSD，未来可能还会涉及 Wayland。这些视为长期目标，目前不打算在当前更新中实现。

#### 待办事项

1. 决定如何处理新旧 xorg 发行版。在最近的 xorg 中，很多旧版驱动程序的支持已去除，因此我们需要维护两个 xorg 发行版，以免失去许多硬件驱动程序。目前，通过在 /etc/make.conf 中设置 WITH_NEW_XORG 标志来完成这一操作，但需要更实用的解决方案。特别重要的是，由于该标志并不非常用户友好，并且目前不会为新发行版提供官方软件包。
