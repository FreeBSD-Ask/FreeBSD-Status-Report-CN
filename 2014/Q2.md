...
2014 年第二季度对于 FreeBSD 项目来说是一个非常忙碌且富有成效的时期。新的 FreeBSD 核心团队已选举产生，FreeBSD Ports 管理团队分支了第二个季度的 “stable” 分支，FreeBSD 发布工程团队正在完成 FreeBSD 9.3-RELEASE 的周期，许多令人兴奋的新功能也加入了 FreeBSD。
...
本季度核心团队的工作涵盖了一些深远的政策审查，以及对项目开发方法论的重大变化。
...
另一个重大变化未获批准。当对改革核心团队选举过程的章程修改进行投票时，该提案未能达到法定人数。

6 月，项目中长时间进行的替换缺陷跟踪系统的项目终于完成。从 6 月 3 日起，FreeBSD 项目已切换到 Bugzilla 作为其缺陷跟踪系统。所有 GNATS PR 的历史记录均已保留，因此无需重新开启旧的工单。目前仍在进行相关工作，以重现以前应用于 GNATS 的一些集成调整，但所有必要的功能均已实现，项目已开始受益于 Bugzilla 带来的新功能。
...
5 月初，FreeBSD 9.3-RELEASE 周期进入了代码软冻结（code slush）阶段。FreeBSD 9.3-RELEASE 周期已接近尾声，9.3-RC3 构建即将开始。9.3-RC3 计划作为此版本周期的最终发布候选版本，在本文撰写时，9.3-RELEASE 应按计划发布。
...
基于 FreeBSD 10.0 发布的新内核 iSCSI 目标和发起栈，Chelsio Communications 开始开发一个卸载接口，以利用 Chelsio T4 和 T5 10G 和 40G 以太网适配器的硬件卸载功能。
...
我们还打算在 Ports 中使用 **pkg(8)**，在软件包构建时处理构建依赖。这样做可以通过下载二进制软件包来安装构建依赖，这意味着可以安装带有自定义选项的软件包，而无需编译其他内容，从而节省时间。
...
此外，正在开发新的官网和论坛，以取代功能有限的老旧网站。新网站将与服务器数据库**链接**，提供有关项目的实时更新。
...
有关该项目更详细的状态报告，可以通过 [stateoftheproject](http://zfsguru.com/news/stateoftheproject/2014) **链接**找到。
...
我们很高兴宣布，基于最近引入到 Ports 框架中的 `USES=uniquefiles` 特性，Python 软件包支持在不同版本的 Python 之间无冲突地并行构建和安装。现在可将 Python 软件包标记为在同一主机上并行为不同 Python 版本构建和安装。然而，构建工具尚不支持此特性，Python 团队将与 `portmgr` 和 pkg 开发人员紧密合作，以在全局 Ports 和 Packages 范围内启用此支持。
...
我们添加了 Python 3.4，移除了 Python 3.1（在其生命周期结束后），更新了 `setuptools` 软件包到 5.1 版本，并将 PyPy 的开发版本更新至 2.3.1。最新的 Python 2.7.8 和更新的 `setuptools` 即将合并到代码库中。
...
由于 Jan Kokemüller 的贡献，现在 Radeon 32 位 ioctl 在 64 位主机上工作正常。这在 FreeBSD 9.x 和 11 上通过 Wine 和《星际争霸 II》得到了成功测试。为使其与 `WITH_NEW_XORG` 一起工作，需要对 **emulators/i386-wine-devel** 进行修改，并创建了新 Port `libtxc_dxtn`，以支持《星际争霸 II》使用的纹理压缩。目前我们还没有时间进一步完善这些内容，因此仍然需要手动操作。
...
自上次报告以来，月度团队一直在忙碌工作，长期的组织者 Gábor Páli 已从团队中退下——感谢 Gábor 所做的所有辛勤工作！这一变化使得报告的准备工作有所滞后，因此发布项目的通知也相对较晚。为了填补这一空缺，Warren Block 和 Benjamin Kaduk 加入了 `monthly@` 团队，与 Glen Barber、Gavin Atkinson、Ed Maste 和其他团队成员一起准备此次报告。特别感谢 Glen 在同时发布 9.3-RELEASE 的过程中，承担了大部分工作！
...
年度董事会会议于 5 月在加拿大渥太华举行。选举了董事和管理人员，并进行了长期规划。我们讨论了我们的愿景、核心价值观、项目路线图和短期目标。我们还与核心团队会面，讨论了角色和责任、项目路线图以及我们能做些什么来帮助项目。
...
一些先前完成的基金会赞助项目得到了增强或附加工作。ARM 超级页项目在去年完成，但现在在 FreeBSD-CURRENT 中默认启用。许多稳定性修复和增强已提交到内核中的 iSCSI 栈。iSCSI 项目已在 FreeBSD 10.0 中发布。许多稳定性修复和增强已提交，并将包含在 FreeBSD 10.1 中。

基金会赞助的工作包括继续进行 autofs 自动挂载守护进程、UEFI 启动支持、更新的 **vt(4)** 系统视频控制台、虚拟机镜像和 Intel 图形驱动更新。在 4 月到 6 月期间，基金会赞助的工作向 FreeBSD 提交了 226 次。
