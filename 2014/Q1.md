...
- Steve Wills 负责维护 Port devel/jenkins-lts。他已经在 jenkins.FreeBSD.org 上实现了多个构建，这些构建检测 FreeBSD Port 仓库中的提交，然后使用 Poudrière 构建 Ports。
...
还进行了许多增强功能，例如对重定向的支持，这是某些高可用性设置所必需的，且在 iscsictl 工具中增加了修改会话参数的能力。以前，必须删除会话并重新添加。
...
Gustau Perez 已经主动承担并在我们开发仓库的 ports-experimental 树中准备了 GNOME 3.12 的 Port 集。我们决定完善 GNOME 3.12，它将在准备工作（大部分）完成且我们对其稳定性满意后进行合并。
...
在图形堆栈的 Port 开发树中，新的 Port Mesa 正在开发中。这些 Port 是支持 GLAMOR（例如用于 Radeon HD 7000+ 显卡的基于 GL 的 2D 加速库）和 OpenCL（使用 GPU 执行非图形计算）所必需的。我们已经能够运行一些 `Hello World` OpenCL 程序，在 darktable 中试用 OpenCL，但在 Clover（Mesa 的 libOpenCL 实现）和 Clang/libc++ 之间有些兼容性问题。
...
ZFS 是 FreeBSD 最重要的特性之一。目前在手册和在线文档中的 ZFS 相关文档严重缺乏。许多来自 Sun 和 Oracle 的原始文档已不再提供，或被移除，或是关于 ZFS 专有版本的内容。
