...
感谢所有报告者的卓越工作！本报告包含了 41 项内容， 希望您喜欢阅读。
...
从 Rui Paulo 的 i386 EFI 加载程序开始，Benno Rice 在 2013 年开发了一个工作中的 amd64 加载程序原型，得到了 FreeBSD 基金会的资助。在经过精炼后，该工作已从 projects/uefi Subversion 分支合并到 FreeBSD head 中。该项目包括构建 UEFI 启用的加载程序所需的基础设施，以及内核端的更改，以解析加载程序提供的元数据。
...
我们还在跟踪核心组件的开发（可以在您的仓库中找到）。请参阅链接以获取如何升级这些库的文档。
...
在 BSDCan 2013 上，开始了撰写全新 ZFS 手册章节的工作。目前该章节约包含 16,000 字，涵盖了 zpool(8) 和 zfs(8) 工具的所有子命令、委托、调优，并且有一个专门的部分定义和解释了 ZFS 的术语和特性。
...
两项基金会资助的项目已完成。第一个项目由 Google 共同赞助，将 Casper 守护进程集成到 FreeBSD 中。第二个项目是针对 FreeBSD 集群的 auditdistd(8) 改进。

基金会资助的项目继续进行：Konstantin Belousov 更新了 Intel 图形驱动程序；Ed Maste 为 amd64 添加了 UEFI 引导支持；Edward Tomasz Napierała 对 autofs 自动挂载程序和内核 iSCSI 堆栈进行了增强和修复；Aleksandr Rybalko 更新了 vt(4) 系统控制台。每个项目的详细更新可以在本季度状态报告中找到。
