...
用于配置网络接口的 ifconfig 程序已进行了全面重构。多年来，ifconfig 已经发展成为一个复杂且经常扭曲的软件，难以理解和维护。此项工作的主要动机是能够进行最小配置（用于嵌入式用途）而无需更改代码，并以模块化的方式支持未来的扩展。功能现在被拆分到单独的文件中，操作注册到中央 ifconfig 代码库。只需指定构建程序时要包含哪些代码即可配置功能。

未来的计划是让 ifconfig 通过动态库自动加载功能。这种机制将允许，例如，第三方软件包提供内核服务和 ifconfig 附加代码，而无需更改基本系统。

### 无线网络支持

联系方式：Sam Leffler <[sam@FreeBSD.org](mailto:sam@FreeBSD.org)>

无线网络层已更新，以支持 802.1x、WPA 和 802.11i 安全协议，以及 WME/WMM 多媒体协议。作为此项工作的一部分，添加了用于加密方法、身份验证和访问控制的可扩展框架。扩展实现为可加载的内核模块，挂载到 net80211 层。例如，此机制用于实现 WEP、TKIP 和 CCMP 加密协议。Atheros 驱动（ath）是目前唯一使用完整功能集的驱动。向其他驱动添加支持很简单，但正在等待志愿者。wpa_supplicant 和 hostapd 程序的移植使得使用新的安全协议成为可能。

对在 bss（托管或 adhoc）中跟踪站点以及扫描时发现的站点的支持进行了全面重构。现在使用多个表，每个表具有不同的管理策略，引用计数现在一致完成，非活动处理更加智能（例如，关联的站点在移除之前会进行探测）。这是迈向正确的漫游支持和其他高级功能的第一步。

添加了 AP 省电支持。关联的站点现在可以在省电模式下运行；发送给它们的帧将在它们休眠时缓冲，多播流量将推迟到下一个信标之后（按照 802.11 协议）。符合标准的接入点需要省电支持。目前只有 ath 驱动实现了省电支持。

正在积极开发中，以添加 Atheros 的 SuperG 功能、WDS，以及在单个设备上支持多 bss（ssid 和/或 bssid）。

#### 未完成任务

1. 除 ath 外的驱动需要更新以支持新的安全协议
2. hostapd 需要工作以支持 IAPP 和 802.11i 预身份验证协议（这些是现有 Linux 代码的简单转换）
