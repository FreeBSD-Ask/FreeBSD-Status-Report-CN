...
- 合并了调试 `debug.mpsafenet` 上的条件巨型锁定到 CVS；语义现在已更改，以使得巨型锁不仅仅是转发路径上起作用，而是对整个栈生效。除非使用我们的补丁，否则需要谨慎使用。回调也被修改为条件安全。
...
如本报告其他部分所述，一些人员继续致力于网络栈的锁定。除了网络栈外，在 3 月和 4 月期间还对以下内容进行了工作。Giant 锁被尽可能向下推送到 `fork`、`exit` 和 `wait` 系统调用中。Alan Cox（alc@）继续锁定虚拟内存子系统，并在适当的位置向下推送 Giant 锁。几个系统调用和回调也标记为 MP 安全。
