...
一个 64 位的 MIPS 模拟器已经出现，提供了在模拟器中测试和调试的能力，并且进行了大量的测试。它已经被加入到 FreeBSD 的 Ports 中，且该 Port 将尽可能积极地跟踪主代码库。一般来说，FreeBSD/MIPS 内核应该能在其中良好运行。
...
HP Network Scanjet 5 可以在扫描仪内部不引人注意地运行 FreeBSD。那些在工作中怀念 Unix 的人可以拥有一台 FreeBSD 主机，不被那些限制大楼内任何非微软 PC 的狭隘经理所注意和阻拦。<http://berklix.com/scanjet/>
...
目前正在进行的工作是将当前在主要 KDE 模块的 Port 中存在的构建开关/选项转换为独立的 Port，以便为它们提供的软件/功能提供软件包。KOffice 1.3.2 的移植工作几乎完成。由 KDE/FreeBSD 团队成员运营和维护的瑞典 FreeBSD 快照服务器 <http://snapshots.se.freebsd.org/> 已恢复运行，且运行良好。额外的 amd64 硬件已被添加，amd64 快照将在不久后提供。
